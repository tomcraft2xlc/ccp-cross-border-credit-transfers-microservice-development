...
@ApplicationScoped
public class VerifyBonificoInWorker {

    private static final Logger LOG = Logger.getLogger(VerifyBonificoInWorker.class);
    ...
    private static String getID(final String resourceName, final UUID resourceID) {
        return VerifyBonificoInWorker.class.getName() + ".verify.transfer:" + resourceName + ":" + resourceID;
    }
    ...
    public void onStart(@Observes final StartupEvent event) {
        LOG.info("Starting VerifyBonificoInWorker");
        jobSubscriber.subscribe(
                Constants.JOB_VERIFY_ACCREDITO,
                new HandleCreditTransferPayload.Deserializer(),
                this.pollService.jobWithPollableData(Constants.JOB_RESTART_POLLABLE_VERIFY_ACCREDITO,
                        this::verifyBonificoIn));
        jobSubscriber.subscribe(
                Constants.JOB_RESTART_POLLABLE_VERIFY_ACCREDITO,
                new PollBucket.Deserializer(),
                this.pollService.jobWithPollableData(Constants.JOB_RESTART_POLLABLE_VERIFY_ACCREDITO,
                        this::restartVerifyBonificoInWithPoll));
        jobSubscriber.subscribe(
                Constants.JOB_RESTART_VERIFY_ACCREDITO,
                new RestartVerifyProcessPayload.Deserializer(),
                this.pollService.jobWithPollableData(Constants.JOB_RESTART_POLLABLE_VERIFY_ACCREDITO,
                        this::restartVerifyBonificoIn));
    }

    public Uni<Void> verifyBonificoIn(final HandleCreditTransferPayload payload) {
        LOG.infof("Starting verify process for %s", payload.idBonificoExtraSepa());
        return this.verifyBonificoIn(payload.idBonificoExtraSepa(), null);
    }

    public Uni<Void> restartVerifyBonificoIn(final RestartVerifyProcessPayload payload) {
        LOG.infof("Restarting verify process for %s", payload.idBonificoExtraSepa());

        return this.verifyBonificoIn(payload.idBonificoExtraSepa(), payload);
    }

    public Uni<Void> restartVerifyBonificoInWithPoll(final PollBucket bucket) {
    ...
    public Uni<Void> verifyBonificoIn(final UUID idBonifico, final RestartVerifyProcessPayload restartProcessInfo) {
